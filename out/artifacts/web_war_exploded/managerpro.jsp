...
<div class="Bott">
    <div class="wrapper clearfix">
        ...
        <div class="you fl"><h2>商品管理</h2>
            <div class="gxin">
                <div align="left" class="xx">
                    <h3 class="clearfix"><strong class="fl">所有商品资料</strong>
                        <a href="addPro.jsp" class="fr" >添加商品</a>
                    </h3>
                   <center>
                    <table border="2" align="center" class="table">
                        ...
                        <c:forEach items="${list }" var="s">
                            <tr>
                                <th>${s.prodouctname }</th>
                                <th>${s.prodouctprice }</th>
                                <th>${s.prodouctstock }</th>
                                <th>
                                    <a href="deleteProServlet?prodouctname=${s.prodouctname }">删除该商品</a></th>
                                <th><a href="toupdateProServlet?prodouctname=${s.prodouctname }">修改该商品信息</a></th>
                            </tr>
                        </c:forEach>
                    </table>
<%--                       <a href="Managerpro?currentPage=${currentPage=1 }">首页</a>--%>
                       <a href='<c:if test="${currentPage>1 }">Managerpro?currentPage=${currentPage-1 }</c:if>'>前一页</a>&nbsp;&nbsp;&nbsp;
                    <c:forEach begin="1" end="${allPage }"  varStatus="st">
                        <a style='<c:if test="${st.count==currentPage}">color:red;</c:if>text-decoration: none' href="Managerpro?currentPage=${st.count }">
                            第${st.count }页&nbsp;&nbsp;&nbsp;&nbsp;
                        </a>
                    </c:forEach>
                       <a href='<c:if test="${currentPage<allPage }">Managerpro?currentPage=${currentPage+1 }</c:if>'>下一页</a>
