...
public class CartDao {
    ...
    public List<Cart> getcartAll(int currentPage,String cart_u_id) throws Exception{
        List<Cart> list=new ArrayList<>();
        JdbcUtil util=new JdbcUtil();
        String sql="select * from cart where cart_valid=0 and cart_u_id=?  limit "+(3*(currentPage-1))+",3";
        PreparedStatement pst=util.getPs(sql);
        pst.setString(1,cart_u_id);
        ResultSet rs = pst.executeQuery();
        while(rs.next()){
            Cart u=new Cart();
            u.setCart_id(rs.getInt("cart_id"));
            u.setCart_p_filename(rs.getString("cart_p_filename"));
            u.setCart_p_name(rs.getString("cart_p_name"));
            u.setCart_p_price(rs.getInt("cart_p_price"));
            u.setCart_nums(rs.getInt("cart_nums"));
            u.setCart_p_id(rs.getInt("cart_p_id"));
            u.setCart_u_id(rs.getString("cart_u_id"));
            u.setCart_valid(rs.getInt("cart_valid"));
    ...
    public List<Cart> getcartAll(int currentPage) throws Exception{
        List<Cart> list=new ArrayList<>();
        JdbcUtil util=new JdbcUtil();
        String sql="select * from cart where cart_valid=0  limit "+(3*(currentPage-1))+",3";
        PreparedStatement pst=util.getPs(sql);
    ...
    public int getCount() throws Exception {
        int count=-1;
        JdbcUtil util = new JdbcUtil();
        String sql = "select count(*) from cart where cart_valid=0";
        PreparedStatement pst = util.getPs(sql);
        ResultSet rs = pst.executeQuery();
    ...
    public int getCount(String cart_u_id) throws Exception {
        int count=-1;
        JdbcUtil util = new JdbcUtil();
        String sql = "select count(*) from cart where cart_valid=0 and cart_u_id= ?";
        PreparedStatement pst = util.getPs(sql);
        pst.setString(1, cart_u_id);
        ResultSet rs = pst.executeQuery();
    ...
    public int getMoney() throws Exception {
        int sum=0;
        JdbcUtil util = new JdbcUtil();
        String sql = "select sum(cart_p_price * cart_nums) FROM cart where cart_valid = 0";
        PreparedStatement pst = util.getPs(sql);
        ResultSet rs = pst.executeQuery();
        if (rs.next()) {
            sum = rs.getInt(1);
